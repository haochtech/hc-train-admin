...
$tablename = $xcmodule . "_" . "service_class";
$do = $_GPC["do"];
...
switch ($op) {
    case "list":
        ...
        if (!empty($_GPC["new"]) && $_GPC["new"] == 1) {
            ...
            include $this->template("old/Service_class/" . $op);
        }
        break;
    case "edit":
        if (!empty($_GPC["id"])) {
    ...
    case "getseachjson":
        ...
        $where = " WHERE uniacid=:uniacid AND type IN (1,2,3) ";
        $params = array();
        if (!empty($_GPC["xname"])) {
