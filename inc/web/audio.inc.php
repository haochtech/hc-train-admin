...
switch ($op) {
    case "list":
        ...
        if (!empty($_GPC["new"]) && $_GPC["new"] == 1) {
            ...
            include $this->template("old/audio/" . $op);
        }
        break;
    case "edit":
        $class = pdo_getall("xc_train_video_class", array("status" => 1, "uniacid" => $uniacid, "type" => 2), array(), '', "sort DESC,createtime DESC");
        if (!empty($_GPC["id"])) {
    ...
    case "record_getseachjson":
        ...
        $where = " WHERE uniacid=:uniacid AND status=1 AND order_type=5 ";
        $params = array();
        if (!empty($_GPC["out_trade_no"])) {
